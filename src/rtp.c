 ...
 * Name: YOUR NAME ZHANG Rushan
 * GTID: YOUR GTID 903849603
 * Fall 2022
 */
...
packet_t *packetize(char *buffer, int length, int *count)
{
    ...
    packets = (packet_t *)malloc(sizeof(packet_t) * (unsigned long)num);
    for (int i = 0; i < num; i++)
    {
        if (i == num - 1)
        {
    ...
    do
    {
        ...
        do
        {
            ...
            if (packet.type == DATA)
            {
                ...
                {
                    // Not sure whether to use static or dynamic memory!!!????!?!?!?!?!
                    packet_t temp;
                    buffer = (char *)realloc(buffer, (unsigned long)(buffer_length + packet.payload_length));
                    memcpy(&(buffer[buffer_length]), packet.payload, packet.payload_length);
                    buffer_length += packet.payload_length;
                    temp.type = ACK;
                    temp.payload_length = 0;
            ...
            else if (packet.type == LAST_DATA)
            {
                ...
                else
                {
                    ...
                    temp.payload_length = 0;
                    net_send_packet(connection->net_connection_handle, &temp);
                    // to make the loop goes one more cycle
                    packet.type = DATA;
                }
            }
...
static void *rtp_send_thread(void *void_ptr)
{
    ...
    do
    {
        ...
        for (i = 0; i < array_length; i++)
        {
            ...
            pthread_mutex_lock(&connection->send_mutex);
            while (connection->is_ack == 0 && connection->is_nack == 0)
            {
                pthread_cond_wait(&connection->send_cond, &connection->send_mutex);
            }
            if (connection->is_ack == 1)
            {
                connection->is_ack = 0;
                connection->is_nack = 0;
            }
            else if (connection->is_nack == 1)
            {
                connection->is_ack = 0;
                connection->is_nack = 0;
                i--;
